...
const LeftSideBar = ({active}: LSBProps) => {
    const setOptActive = (target:  HTMLElement) => {
        console.log(target);
        document.getElementsByClassName('lsoptionactive')[0].className = 'lsoptionnormal';
        if(target.className === 'lsoptionnormal'){
            target.className = 'lsoptionactive'
        }else{
            let parent: any = target.parentElement
            parent.className = 'lsoptionactive';
        }
    }

    return(
        <div id="leftsidebar" style={{left: window.innerWidth<1024?active:''}} >
            ...
            <div className="lsoptionactive" onClick={(e)=>{setOptActive(e.target as HTMLElement);}}>
                <img className="leftsideimg" alt="" src={users}/>
                <div className="lsotext">Users</div>
            </div>
            <div className="lsoptionnormal" onClick={(e)=>{setOptActive(e.target as HTMLElement);}}>
